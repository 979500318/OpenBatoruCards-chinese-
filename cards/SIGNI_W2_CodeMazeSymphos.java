...
public final class SIGNI_W2_CodeMazeSymphos extends Card {
    
    public SIGNI_W2_CodeMazeSymphos()
    {
        setImageSets("WXDi-P02-052");
        
        setOriginalName("コードメイズ　シンフォーズ");
        setAltNames("コードメイズシンフォーズ Koodo Meizu Shinfoozu");
        setDescription("jp",
                "@U：対戦相手のシグニ１体がアタックしたとき、その正面にシグニがない場合、このシグニをアタックしたそのシグニの正面に配置してもよい。" +
                "~#：対戦相手のシグニ１体を対象とし、ターン終了時まで、それは@>@C@#：アタックできない。@@@@を得る。カードを１枚引く。"
        );
        
        setName("en", "Symphorce, Code: Maze");
        setDescription("en",
                "@U: Whenever a SIGNI on your opponent's field attacks, if there is no SIGNI in front of the attacking SIGNI, you may move this SIGNI to the SIGNI Zone in front of the attacking SIGNI." +
                "~#Target SIGNI on your opponent's field gains@>@C@#: This SIGNI cannot attack.@@@@until end of turn. Draw a card."
        );
        
        setName("en_fan", "Code Maze Symphos");
		...
		setName("zh_simplified", "迷宫代号 三得利音乐厅");
        setDescription("zh_simplified", 
                "@U :当对战对手的精灵1只攻击时，那只的正面没有精灵的场合，可以把这只精灵往攻击中的那只精灵的正面配置。" +
    ...
    public class IndexedInstance extends Card.IndexedInstance
    {
        ...
        private void onAutoEff(CardIndex caller)
        {
            ...
            {
                moveToZone(getCardIndex(), CardLocation.getOppositeSIGNILocation(caller.getLocation()));
            }
        }
